...
function pmprowoo_get_membership_price( $price, $product ) {
	...
	// ignore membership products and subscriptions if we are set that way @pbrocks
	...
	// use this level to get the price @pbrocks
	...
	if ( count( $order->get_items() ) > 0 ) {
		foreach ( $order->get_items() as $item ) {
			if ( $item['type'] == 'line_item' ) {
				// get product info and check if product is marked to autocomplete @pbrocks
